...
4. Chat box continues as that user/thread: append turns, retrieve again, keep
   ...
   retrieved memory context (gemini-3.1-flash-lite by default).

The page shell, layered retrieval, budget metrics, and grounded Gemini chat are
wired end to end through `retrieve_for_case(...)`.
...
def retrieve_for_case(
    ...
    """Run the student retrieval pipeline for a loaded evaluation case.
    ...
    if expected_layer == "mixed":
        selected = case.get("retrieve_layers") or ["long_term", "semantic"]
    else:
        selected = [expected_layer]

    source_messages = case.get("fixture_messages")
        ...
        try:
            ...
            if gemini_available():
                try:
                    ...
                    st.warning(
                        f"Gemini reply unavailable ({type(llm_exc).__name__}); "
                        "showing retrieved context instead."
                    )
                    reply = "_(LLM unavailable — retrieved context)_\n\n" + (
